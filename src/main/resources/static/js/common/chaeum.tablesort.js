const TableSort = (function() {
	...
	TableSort.prototype.createSortableElement = function(header, selector) {
		...
		button.setAttribute("column", header.column);
		//초기값 세팅하기위해 추가
		if(!isNull(header.default)) button.dataset.order = header.default;
	...
	TableSort.prototype.setEventListener = function(button, header, textContent) {
		...
		button.addEventListener("click", function(e) {
			const order = e.currentTarget.dataset.order === ORDER.DESC.text ? ORDER.ASC : ORDER.DESC;
			_this.resetHeaders(textContent);
			_this.click({ target: button, column: header.column, order: order.text, orderQuery: header.column + " " + order.text });
	...
	TableSort.prototype.init = function(obj) {
		...
		target.tag.textContent = target.tag.textContent + (obj.order === ORDER.DESC.text ?  ORDER.ASC.icon : ORDER.DESC.icon);
		target.tag.click();
	}
